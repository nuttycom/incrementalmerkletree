...
pub struct BatchInsertionResult<H, C: Ord, I: Iterator<Item = (H, Retention<C>)>> {
    ...
    /// The positions of all leaves with [`CHECKPOINT`](RetentionFlags::CHECKPOINT) retention
    /// that were inserted.
...
impl<H: Hashable + Clone + PartialEq> LocatedPrunableTree<H> {
    ...
    /// roots corresponding to later positions will be filled by
    /// [`H::empty_root`](Hashable::empty_root).
    ///
    ...
    /// * `prune_below` - Nodes with [`EPHEMERAL`](RetentionFlags::EPHEMERAL) retention that
    ///   are not required to be retained in order to construct a witness for a marked node or to
    ///   make it possible to rewind to a checkpointed node may be pruned so long as their address
    ///   is at less than the specified level.
